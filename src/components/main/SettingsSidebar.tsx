        ...
        <div className="hidden lg:block">
            ...
            <div className="hidden md:block">
                <AccordionSection title="General Settings" isOpen={openSections.general} setIsOpen={() => toggleSection('general')}>
                    <FormRow label="Content Type"><select value={settings.selectedContentType} onChange={e => updateSetting('selectedContentType', e.target.value as ContentType)} className="w-full p-1.5 border rounded-md text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{Object.values(ContentType).map(type => <option key={type} value={type}>{type.charAt(0).toUpperCase() + type.slice(1)}</option>)}</select></FormRow>
                    <FormRow label="Category"><select value={settings.selectedContentCategory} onChange={e => updateSetting('selectedContentCategory', e.target.value as ContentCategoryValue)} className="w-full p-1.5 border rounded-md text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{Object.values(ContentCategory).map(cat => <option key={cat} value={cat}>{cat}</option>)}</select></FormRow>
                    <FormRow label="Emotion"><select value={settings.selectedEmotion} onChange={e => updateSetting('selectedEmotion', e.target.value as SelectedEmotion)} className="w-full p-1.5 border rounded-md text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{Object.entries(Emotions).map(([key, name]) => <option key={key} value={key}>{name}</option>)}</select></FormRow>
                    <FormRow label="Language"><select value={settings.selectedLanguage} onChange={e => updateSetting('selectedLanguage', e.target.value as SelectedLanguageCode)} className="w-full p-1.5 border rounded-md text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{Object.entries(LanguageOptions).map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></FormRow>
                    <FormRow label="Country"><select value={settings.selectedCountryCode} onChange={e => updateSetting('selectedCountryCode', e.target.value as SelectedCountryCode)} className="w-full p-1.5 border rounded-md text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">{Object.entries(CountryOptions).map(([code, name]) => <option key={code} value={code}>{name}</option>)}</select></FormRow>
            ...
            <AccordionSection title="Summary & Sources" isOpen={openSections.summary} setIsOpen={() => toggleSection('summary')}>
                ...
                <FormRow label="Text Color">
                    <CustomColorPicker value={stylesForSidebar.summaryTextColor} onChange={c => handleStyleChange('summaryTextColor', c)} isOpen={activeColorPicker === 'summaryText'} onToggle={() => handleColorPickerToggle('summaryText')} />
                </FormRow>
                <FormRow label="BG Type">
                    <div className="flex rounded-md shadow-sm w-full">
                        <button onClick={() => handleStyleChange('summaryType', HeaderType.Solid)} className={`px-3 py-1.5 text-xs w-full rounded-l-md ${stylesForSidebar.summaryType === HeaderType.Solid ? 'bg-primary text-primary-text' : 'bg-white dark:bg-gray-700'}`}>Solid</button>
                        <button onClick={() => handleStyleChange('summaryType', HeaderType.Gradient)} className={`px-3 py-1.5 text-xs w-full rounded-r-md ${stylesForSidebar.summaryType === HeaderType.Gradient ? 'bg-primary text-primary-text' : 'bg-white dark:bg-gray-700'}`}>Gradient</button>
                    </div>
                </FormRow>
                {stylesForSidebar.summaryType === HeaderType.Solid ? (
                    <FormRow label="BG Color">
                        <CustomColorPicker value={stylesForSidebar.summarySolidColor} onChange={c => handleStyleChange('summarySolidColor', c)} isOpen={activeColorPicker === 'summarySolid'} onToggle={() => handleColorPickerToggle('summarySolid')} />
                    </FormRow>
                ) : (
                    <>
                        <FormRow label="Direction">
                            <select value={stylesForSidebar.summaryGradientDirection} onChange={e => handleStyleChange('summaryGradientDirection', e.target.value as GradientDirectionType)} className="w-full p-1.5 border rounded-md text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
                                {Object.entries(GradientDirectionOptions).map(([key, name]) => <option key={key} value={key}>{name}</option>)}
                            </select>
                        </FormRow>
                        <FormRow label="Color 1">
                            <CustomColorPicker value={stylesForSidebar.summaryGradientColor1} onChange={c => handleStyleChange('summaryGradientColor1', c)} isOpen={activeColorPicker === 'summaryGrad1'} onToggle={() => handleColorPickerToggle('summaryGrad1')} />
                        </FormRow>
                        <FormRow label="Color 2">
                            <CustomColorPicker value={stylesForSidebar.summaryGradientColor2} onChange={c => handleStyleChange('summaryGradientColor2', c)} isOpen={activeColorPicker === 'summaryGrad2'} onToggle={() => handleColorPickerToggle('summaryGrad2')} />
                        </FormRow>
                    </>
                )}
            </AccordionSection>
