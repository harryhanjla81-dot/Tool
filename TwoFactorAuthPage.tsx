...
const TwoFactorAuthPage: React.FC = () => {
    ...
    useEffect(() => {
        const updateCodesAndTimer = async () => {
            ...
            // Regenerate codes only when the 30s window resets or if the number of accounts has changed
            if (timeLeft === 30 || Object.keys(codes).length !== accounts.length) {
                const newCodes: Record<string, GeneratedCode> = {};
                // Use Promise.all for concurrency
                await Promise.all(accounts.map(async (account) => {
                    try {
                        const secretBytes = base32ToBytes(account.secret);
                        if (secretBytes.length === 0) throw new Error("Invalid secret");
                ...
                // Just update timeLeft for existing codes
                setCodes(prev => {
                    const updated = { ...prev };
                    for (const key in updated) {
                        updated[key] = { ...updated[key], timeLeft };
                    }
                    return updated;
                });
            }
        ...
        updateCodesAndTimer(); // Initial call
        const intervalId = setInterval(updateCodesAndTimer, 1000);
        return () => clearInterval(intervalId);
    }, [accounts, codes]);

    // Effect for immediate code generation
    useEffect(() => {
        const updateImmediateCode = async () => {
            const epoch = Math.floor(Date.now() / 1000);
            const timeLeft = 30 - (epoch % 30);
            ...
            // Only generate a new code when the time step changes, or if there's no code yet.
            if (timeLeft === 30 || !immediateCode || immediateCode === 'Invalid Key') {
                try {
                    const secretBytes = base32ToBytes(immediateSecret);
                    if(secretBytes.length === 0) throw new Error("Invalid key");
                    const code = await totp(secretBytes);
        ...
        updateImmediateCode();
        const intervalId = setInterval(updateImmediateCode, 1000);
        return () => clearInterval(intervalId);
    }, [immediateSecret, immediateCode]);
