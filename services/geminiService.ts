...
const getAiClient = (): GoogleGenAI => {
    if (!activeApiKey) {
        throw new Error("Gemini API Key is not configured. Please add one in Settings > API Keys.");
    }
...
function handleApiError(error: any, context: string): Error {
    ...
    if (rawMessage.includes("API key not valid") || rawMessage.includes("API_KEY_INVALID")) {
        return new Error("An invalid Google API Key was provided. Please check your keys in Settings > API Keys.");
    }
...
export async function fetchContentFromPrompt(
  ...
  emotion: SelectedEmotion
    ...
    const languageInstruction = languageName === 'Hindi' 
        ? `
HINDI LANGUAGE INSTRUCTIONS:
- Write in clear, simple formal Hindi that is easy for all audiences to understand.
- Avoid overly Sanskritized vocabulary. Prefer everyday words (e.g., “upar kamayi” over “āya”, “vyavsaay”/“business” over “udhyog” when natural).
- Keep sentences short, in active voice, and reader-friendly.
- Hinglish words (like 'startup', 'marketing') are allowed only if they improve clarity and feel natural.
`
        : '';
...
export async function regenerateHeadlineByWordCount(
  ...
  const languageInstruction = languageName === 'Hindi'
      ? `- Use clear, simple formal Hindi. Avoid overly Sanskritized words. Use active voice.`
      : '';
...
HINDI/HINGLISH LANGUAGE INSTRUCTIONS:
- Write in clear, simple formal Hindi that is easy for all audiences to understand.
- Avoid overly Sanskritized vocabulary. Prefer everyday words.
- Keep sentences short and reader-friendly. Hinglish words are encouraged for a viral, natural feel.
...
- Write in clear, simple formal Hindi that is easy for all audiences to understand.
- Avoid overly Sanskritized vocabulary. Prefer everyday words.
- Keep sentences short and reader-friendly. Natural Hinglish words are okay.
`
        : '';
...
- Write in clear, simple formal Hindi that is easy for all audiences to understand.
- Avoid overly Sanskritized vocabulary. Prefer everyday words.
- Keep sentences short, active voice, and reader-friendly. Natural Hinglish words are okay.
`
        : '';
    const prompt = `
...
export async function generatePostFromTopics(topics: string[]): Promise<string> {
    ...
    const main topics to include are: ${topics.join(', ')}.
...
Modern Hinglish & Clarity: Write in clear, simple formal Hindi that is easy for all audiences to understand. Avoid overly Sanskritized vocabulary. Prefer everyday words. Keep sentences short and in active voice. Use natural, conversational Hindi that seamlessly integrates commonly spoken English words written in Devanadari script (e.g., प्रॉब्लम, सिचुएशन, शॉकिंग, प्लान, मिशन).
