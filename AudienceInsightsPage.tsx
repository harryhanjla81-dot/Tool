                    ...
                    topPosts = latestPosts.map((post, index) => {
                        ...
                        if (res?.code === 200) {
                             try {
                                ...
                                const likesRaw = post.likes?.summary.total_count;
                                const commentsRaw = post.comments?.summary.total_count;
                                const likes = typeof likesRaw === 'number' ? likesRaw : 0;
                                const comments = typeof commentsRaw === 'number' ? commentsRaw : 0;

                                return { id: post.id, message: post.message || 'No caption', thumbnail: post.full_picture || '', reach, engagement, likes, comments };
                            } catch(e) { /* ignore */ }
                        }
                        return null;
