...
const SettingsBottomSheet: React.FC<{ title: string; onClose: () => void; children: React.ReactNode; }> = ({ title, onClose, children }) => {
    return (
        <>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white/30 dark:bg-gray-900/60 backdrop-blur-md rounded-t-xl shadow-lg z-50 md:hidden max-h-[80vh] flex flex-col transition-transform duration-200">
...
const MobileFooterToolbar: React.FC<{
    ...
    const FooterButton: React.FC<{ icon: React.ReactNode; label: string; value?: string; onClick: () => void }> = ({ icon, label, value, onClick }) => (
        <button onClick={onClick} className="flex flex-col items-center justify-center gap-1 flex-shrink-0 w-20 text-center p-2 rounded-lg bg-white/3 dark:bg-gray-800/40 border border-white/8 hover:bg-white/6 transition-colors">
            <div className="text-gray-900 dark:text-gray-100">{icon}</div>
            <span className="text-xs font-semibold text-gray-900 dark:text-gray-100 truncate w-full">{label}</span>
            {value && <span className="text-[10px] text-gray-500 truncate w-full">{value}</span>}
        </button>
    );

    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white/6 dark:bg-gray-900/60 backdrop-blur-md border-t border-gray-200/8 z-30 md:hidden rounded-t-xl shadow-lg">
            <div className="flex items-center gap-1 p-1">
                <div className="relative flex-grow flex items-center overflow-hidden">
                    {showScrollArrows.left && <div className="absolute left-0 top-0 bottom-0 z-10 w-8 bg-gradient-to-r from-black/20 to-transparent flex items-center justify-start pointer-events-none"><ChevronLeftIcon className="w-5 h-5 text-gray-500/70" /></div>}
                    ...
                    {showScrollArrows.right && <div className="absolute right-0 top-0 bottom-0 z-10 w-8 bg-gradient-to-l from-black/20 to-transparent flex items-center justify-end pointer-events-none"><ChevronRightIcon className="w-5 h-5 text-gray-500/70" /></div>}
                </div>
    ...
    useEffect(() => {
        setHeaderActions(
            <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-800 dark:text-gray-100 bg-white/6 dark:bg-gray-800/60 border border-white/10 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/30"
    ...
    const renderSheetContent = () => {
        switch (activeSheet) {
            ...
            case 'summary': return <div className="space-y-4">
                ...
                <hr className="my-4 border-gray-300 dark:border-gray-600" />
                <p className="text-base font-semibold -mb-2">Background Style</p>
                <FormRow label="Text Color">
                    <CustomColorPicker label="Summary text color" value={stylesForMobile.summaryTextColor} onChange={c => handleMobileStyleChange('summaryTextColor', c)} isOpen={activeColorPicker === 'summaryText'} onToggle={() => setActiveColorPicker(p => p === 'summaryText' ? null : 'summaryText')} />
                </FormRow>
                <FormRow label="BG Type">
                    <div className="flex rounded-md shadow-sm w-full">
                        <button onClick={() => handleMobileStyleChange('summaryType', HeaderType.Solid)} className={`px-3 py-1.5 text-xs w-full rounded-l-md ${stylesForMobile.summaryType === HeaderType.Solid ? 'bg-primary text-primary-text' : 'bg-white dark:bg-gray-700'}`}>Solid</button>
                        <button onClick={() => handleMobileStyleChange('summaryType', HeaderType.Gradient)} className={`px-3 py-1.5 text-xs w-full rounded-r-md ${stylesForMobile.summaryType === HeaderType.Gradient ? 'bg-primary text-primary-text' : 'bg-white dark:bg-gray-700'}`}>Gradient</button>
                    </div>
                </FormRow>
                {stylesForMobile.summaryType === HeaderType.Solid ? (
                    <FormRow label="BG Color">
                        <CustomColorPicker label="Summary background color" value={stylesForMobile.summarySolidColor} onChange={c => handleMobileStyleChange('summarySolidColor', c)} isOpen={activeColorPicker === 'summarySolid'} onToggle={() => setActiveColorPicker(p => p === 'summarySolid' ? null : 'summarySolid')} />
                    </FormRow>
                ) : (
                    <>
                        <FormRow label="Direction">
                            <select value={stylesForMobile.summaryGradientDirection} onChange={e => handleMobileStyleChange('summaryGradientDirection', e.target.value as GradientDirection)} className="w-full p-2 border rounded-md text-sm">
                                {Object.entries(GradientDirectionOptions).map(([key, name]) => <option key={key} value={key}>{name}</option>)}
                            </select>
                        </FormRow>
                        <FormRow label="Color 1">
                            <CustomColorPicker label="Summary gradient color 1" value={stylesForMobile.summaryGradientColor1} onChange={c => handleMobileStyleChange('summaryGradientColor1', c)} isOpen={activeColorPicker === 'summaryGrad1'} onToggle={() => setActiveColorPicker(p => p === 'summaryGrad1' ? null : 'summaryGrad1')} />
                        </FormRow>
                        <FormRow label="Color 2">
                            <CustomColorPicker label="Summary gradient color 2" value={stylesForMobile.summaryGradientColor2} onChange={c => handleMobileStyleChange('summaryGradientColor2', c)} isOpen={activeColorPicker === 'summaryGrad2'} onToggle={() => setActiveColorPicker(p => p === 'summaryGrad2' ? null : 'summaryGrad2')} />
                        </FormRow>
                    </>
                )}
            </div>;
